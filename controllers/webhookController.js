...
const handlePaystackWebhook = async (req, res) => {
  try {
    ...
    if (!secret) {
      console.error(
        "[webhook] PAYSTACK_SECRET not configured — rejecting request",
      );
    ...
    if (hostingOrder) {
      ...
      if (
        wasPaidOrActive &&
        hostingOrder.provisioningStatus !== "not_started"
      ) {
        console.log(
    ...
    if (domainOrder) {
      ...
      if (registrationSucceeded && domainOrder.user) {
        try {
          ...
          // Use domainOrder.user (ObjectId) — no email lookup needed
          ...
          console.error(
            "[webhook] Failed to link domain to user account:",
            err.message,
          );
        }
      }
