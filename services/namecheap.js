...
async function getPricing() {
  ...
  try {
    ...
    const response = await axios.get(`${getBaseUrl()}?${qs}`, {
      timeout: 15000,
    });
    ...
    try {
      ...
      chunk.forEach((d) => {
        ...
        results.push({
          domain: d,
          available: false,
          price: livePrices[tld] ?? getDefaultPrice(tld),
          error: err.message,
        });
      });
    }
...
async function registerDomain(domain, years, registrant, options = {}) {
  ...
  try {
    ...
    return {
      success: false,
      error: err.message || "Domain registration failed",
    };
  }
}
